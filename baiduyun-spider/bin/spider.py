# -*- coding: utf-8 -*-
import urllib2, re, argparse, json, time,requests
...
def getHtml(url, ref=None, reget=5):
    try:
        # proxy_info = {'host' : '45.77.63.159','port' : 1189}
...
def getHtmlWap(url, ref=None, reget=5):
    headers = {'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:22.0) Gecko/20100101 Firefox/22.0'}
    content = requests.get(url,headers=headers).content
    pattern = re.compile(r'window.yunData = (.*})')
...
class BaiduPanSpider(object):
    def __init__(self):
        ...
        self.while_count = 0
        self.spider_queue = Queue.Queue(maxsize=100)
        self.status = 'stop'
        self.errno = ERR_NO
    ...
    def getShareListsWap(self, uk, start=0):
        ...
        total_count = feedata['total_count']
        returns = []
        count = 0
        if total_count > 0:
            for item in feedata['records']:
                count = count + 1
                feed_type = item['feed_type']
                isdir = 0
                size = 0
    ...
    def startSpiderWap(self):
        ...
        while not self.spider_queue.empty():
            ...
            if not share_user['file_done']:
                print '%d now spidering file ,%d  file fetched' % (share_user['uk'], share_user['file_fetched'])
                rs = self.getShareListsWap(share_user['uk'], share_user['file_fetched'])
                if not rs:
                    print 'uk:%d error to fetch files,try again later...' % share_user['uk']
                    return True
                total_count, fetched_count, file_list = rs
